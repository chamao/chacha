...
<div class="container" style="padding-top:70px;width:100%;">
  <div class="col-xs-12 col-sm-10 col-lg-8 center-block" style="float: none;">
    ...
    if (isset($_GET['name'])) {
      ...
      $gls = $DB->count("SELECT count(*) from product WHERE{$sql}");
      $con = '名称 ' . $_GET['name'] . ' 共有 <b>' . $gls . '</b> 个商品';
    } else {
      $gls = $DB->count("SELECT count(*) from product WHERE 1");
      $sql = " 1";
      $con = '后台共有 <b>' . $gls . '</b> 个商品';
    }
    ...
    <div class="table-responsive">
      <table class="table table-striped">
        ...
        <tbody>
          <?php
          $rs = $DB->query("SELECT * FROM product WHERE{$sql} order by id");
          while ($res = $DB->fetch($rs)) {
             $resume = mb_substr($res['resume'],0,18)."....";
              $introduce = mb_substr($res['introduce'],0,30)."....";
            echo '<tr><td>' . $res['id'] . '</td><td>' . $res['name'] . '</td><td>' . $res['price'] . '</td><td>' . $resume . '</td><td>' . $introduce. '</td><td>' . $res['pic'] . '</td><td>' . $res['date'] . '</td><td><a href="./edit.php?my=update&id=' . $res['id'] . '" class="btn btn-xs btn-info">修改</a> <a href="./edit.php?my=del&id=' . $res['id'] . '" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除这个商品吗？\');">删除</a></td></tr>';
            
            
            //   echo '<tr><td>' . $res['id'] . '</td><td>' . $res['name'] . '</td><td>' . $res['price'] . '</td><td>' . $res['resume'] . '</td><td>' . $res['introduce']. '</td><td>' . $res['pic'] . '</td><td>' . $res['date'] . '</td><td><a href="./edit.php?my=update&id=' . $res['id'] . '" class="btn btn-xs btn-info">修改</a> <a href="./edit.php?my=del&id=' . $res['id'] . '" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除这个商品吗？\');">删除</a></td></tr>';
          }
          ?>
        </tbody>
      </table>
